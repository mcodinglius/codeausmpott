...
export function LearningRequestBuilder() {
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        if (selectedTopics.length === 0) {
            setStatus("error");
            setMessage("Zieh zuerst mindestens ein Thema in dein Lern-Gehirn.");
            return;
        }
        ...
        try {
            ...
            form.reset();
            setStatus("success");
            setMessage("Deine Anfrage wurde erfolgreich gesendet.");
        } catch (error) {
            console.error(error);
            setStatus("error");
            setMessage(
    ...
    return (
        <DragDropProvider
            onDragEnd={(event) => {
                ...
                if (String(target.id) !== BRAIN_ID) return;
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-14">
                ...
                <section>
                    ...
                    <p className="mt-3 max-w-2xl leading-7 text-[var(--color-text-soft)]">
                        Wähle einen Bereich und zieh die gewünschten Themen in dein
                        Lern-Gehirn. Auf dem Smartphone kannst du einfach auf + tippen.
                    </p>
                ...
                <section className="space-y-8 border-t border-[var(--color-border)] pt-10">
                    ...
                    <div>
                        ...
                        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                            {["Schule", "Ausbildung", "Studium", "Eigenes Interesse"].map(
                                (context) => (
                                    <label
                                        key={context}
                    ...
                    <div className="grid gap-5 lg:grid-cols-2">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                name="goal"
                                rows={6}
                                placeholder="z. B. Klausur bestehen, Java wirklich verstehen oder mehr Spaß am Programmieren bekommen"
                                className="w-full resize-none rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-strong)] p-4 leading-7 text-[var(--color-text)] outline-none transition placeholder:text-[var(--color-text-soft)] focus:border-[var(--color-primary)] focus:ring-4 focus:ring-[rgba(24,1,97,0.08)]"
                            />
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <p className="font-semibold text-[var(--color-text)]">
                                    ...
                                    {selectedTopics.length === 1 ? "Thema" : "Themen"} im Gehirn
                                </p>
                                <p className="mt-1 text-sm text-[var(--color-text-soft)]">
                                    Genau diese Lerninhalte werden mit deiner Anfrage übermittelt.
                                </p>
                            </div>
            ...
            <DragOverlay dropAnimation={null}>
                {(source) => {
                    ...
                    const topicId = String(source.id).replace("topic:", "");
                ...
                <div>
                    ...
                    <p className="mt-1 text-sm text-[var(--color-text-soft)]">
                        {topics.length === 0
                            ...
                            : `${topics.length} ${
                                topics.length === 1 ? "Lerninhalt" : "Lerninhalte"
                            } gesammelt`}
                    </p>
                </div>
            ...
            >
                <div
                    className={`pointer-events-none absolute inset-10 rounded-[inherit] blur-3xl transition-opacity duration-300 ${
                        isDropTarget ? "bg-[rgba(235,54,120,0.16)] opacity-100" : "opacity-0"
                    }`}
                />
                    ...
                    {topics.length === 0 ? (
                        <div className="mx-auto max-w-xs text-center">
                            ...
                            <p className="mt-2 text-sm leading-6 text-[var(--color-text-soft)]">
                                Alles, was du lernen möchtest, landet hier und wird Teil deiner
                                Anfrage.
                            </p>
                        </div>
                    ) : (
